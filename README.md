...
| **[`greptile-resolve`](skills/greptile-resolve/)** | **One-pass**, Greptile-specific response to a PR review. Reads the inline comments + Confidence Score, verifies each against the actual code, then for each: if valid, applies the minimal fix and **marks the review thread resolved** via GraphQL; if invalid, replies with a brief justification and leaves the thread open. Pushes the fixes, re-requests review, **exits** — does not poll. Use this when `/review-pr`'s plan-then-approve gate is overkill and you trust the agent to decide validity itself. | `/greptile-resolve <n>` |
