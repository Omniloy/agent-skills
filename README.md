...
skills/
  ...
  backlog/   ← creating & orchestrating work (tech-lead / planning)
    ...
    feature-to-tasks         decompose a Jira Feature into the port tasks that deliver it
...
| backlog | **[`feature-to-tasks`](skills/backlog/feature-to-tasks/)** | Turns a Jira **Feature** into the few technical tasks that deliver it. The product already exists as unofficial code; these tasks bring it into the audited monorepo, **one `Tarea` per package, three to five per Feature, never thirty**. Each carries *Requisito · Punto de partida · Alcance IN/OUT · Destino · Criterios de aceptación · Comprobación · Riesgos ISO 14971*, `Relates`-linked to the Feature and to the Verification that will prove it. `### Punto de partida` declares the kind — **`Porte`** (the code moves), **`Reescritura`** (the behaviour survives, the architecture changes, citing the decision that says so) or **`Nuevo`** (nothing behind it) — names the real repo and paths, verified against the clone and never guessed, and says what has to change on the way (shared contracts, explicit tenancy, no PHI). The kind sets the parity criterion, which is where getting it wrong is expensive. Also `refinar`: section-by-section diff plus a Jira comment, never a silent rewrite, never widens `IN`. **Never assigns anyone.** | `/feature-to-tasks <KEY>` |
