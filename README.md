...
skills/
  ...
  backlog/   ← creating & orchestrating work (tech-lead / planning)
    ...
    feature-to-tasks         decompose a Jira Feature into standard technical tasks
...
| backlog | **[`feature-to-tasks`](skills/backlog/feature-to-tasks/)** | Turns a Jira **Feature** into the **technical tasks** that implement it, to one fixed shape: one `Tarea` per monorepo package, each carrying *Requisito · Funcional · Enfoque técnico · Alcance IN/OUT · Puntos de entrada · Contrato · Criterios de aceptación · Comprobación · Riesgos ISO 14971*, hung off its Epic and `Relates`-linked to the Feature. Templates live in `references/` as literal files so ~150 tickets come out identical instead of drifting. **Dry-run by default**: the batch goes to a file, `scripts/check_ready.py` enforces a mechanical Definition of Ready, and one failure aborts the whole batch. Also `refine` (section-by-section diff + Jira comment, never a silent rewrite, never widens `IN`) and `audit` (re-validate the live board). **Never assigns anyone.** | `/feature-to-tasks <KEY>` |
